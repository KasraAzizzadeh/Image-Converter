...
multiply_single:
	...
	mov rcx, qword [numsq]		    ;;loop counter for multiplication = n^2
	mov rsi, 0			    ;;index for matrix iteration
	
	loopS:
		sub rcx, 1		    ;;update loop counter
		movss xmm0, [KERNEL + rsi]  ;;put float in offset + index of kernel into xmm0
		mulss xmm0, [INVERS + rsi]  ;;mul float in offset + index of inverse into xmm0
		add rsi, 4		    ;;update index from offset
		addss xmm0, [sum]
		movss [sum], xmm0 	    ;;get sum of all n^2 multiplications
...
convolution:
	...
	mov [RESULT + r12 * 4], eax		;;put convolution product in result
        ...
        sub r12, 1				;;update loop counter to go to next pixel
        cmp r12, 0
        jge loopC
...
print_result:
	...
	mov r12, qword [allPixels]		    ;;set loop count = number of pixels
	xor rax, rax
	mov rax, qword [columns]
	mov r15, 4
	mul r15
	mov r15, rax
	mov r14, r15		    	    ;;set index contion = x elements * 4 bytes
	mov rbp, 0			    ;;set index for matrix iteration
	jmp loopR	

	pre_loopR:
		call print_nl		    
		add r14, r15 	    	    ;;update index condition

	loopR:
		cmp rbp, r14		    ;;if reached end of row go to next line
		je  pre_loopR
		sub r12, 1		    ;;update loop condition
		mov edi, dword  [RESULT + rbp]     ;;get element at offset + index
		call print_float	    ;;print element
		mov edi, 32
